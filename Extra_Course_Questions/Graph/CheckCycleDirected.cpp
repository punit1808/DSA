...
class Graph{
    ...
    bool DirectedCheckCycleUsingDFS(int source,unordered_map<int,bool>& visited,
    unordered_map<int,bool> dfsvisited){
        visited[source]=true;
        dfsvisited[source]=true;
        for(auto i:map[source]){
            if(!visited[i]){
                bool ans=DirectedCheckCycleUsingDFS(i,visited,dfsvisited);
            if(ans){
                return true;
            }
            }
...
int main(){
    ...
    // Cycle is Absent->Directed

    g.Insertion(0,1,1);
    g.Insertion(0,2,1);
    g.Insertion(1,2,1);

    // Cycle is Present->Directed
    ...
    bool ans=g.DirectedCheckCycleUsingDFS(0,visited2,dfsVisited);
    if(ans){
